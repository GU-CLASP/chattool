 ...
 public class TubeIn_InsertingFakeText_Balloon_HahaAdd
   extends Thread
   implements WYSIWYGTube
 {
   TubeOut to;
   ...
   public String[] queries = new String[] { " haha ", " hahaha ", " :D " };
   
   public int interventionMinPreInterventionGap = 50;
   public int interventionMaxPreInterventionGap = 150;
   ...
   public TubeIn_InsertingFakeText_Balloon_HahaAdd(Dyadic_WYSIWYGInterface_Manipulation dw) { this.r = new Random(); this.dw = dw; this.to = new TubeOut(dw); boolean dosetup = CustomDialog.getBoolean("Do you want to customize the intervention settings?", "Customize", "Use default");
     if (dosetup)
       doSetup(); 
     start(); } public void addFakeText(Participant fakeSender, String text, int minPreInterventionGap, int maxPreInterventionGap, int minIntraWordGap, int maxIntraWordGap, int minInterWordGap, int maxInterWordGap) { if (text == null)
       return;  if (text.equalsIgnoreCase("")) {
       return;
     }
     long gap = 0L;
   ...
   public void detectTarget(Participant sender) {
     Vector<MessageWYSIWYGDocumentSyncFromClientInsert> participantsText = this.htEachParticipantsText.get(sender);
     if (participantsText == null)
       return;  String previousText = ""; int i;
     for (i = participantsText.size() - 1; i >= 0 && i >= participantsText.size() - 100; i--) {
       ...
       if (mWYSIWYGInsert.getTextTyped().equalsIgnoreCase(" "))
         break;  previousText = mWYSIWYGInsert.getTextTyped() + previousText;
     } 
     ...
     for (i = 0; i < this.targets.length; i++) {
       ...
       if (previousText.endsWith(target)) {
         ...
         if ((new Date()).getTime() - this.timeOfLastTrigger > this.minGapBtwnInter) {
           addFakeText(sender, query, this.interventionMinPreInterventionGap, this.interventionMaxPreInterventionGap, this.interventionMinIntraWordGap, this.interventionMaxIntraWordGap, this.interventionMinInterWordGap, this.interventionMaxInterWordGap);
